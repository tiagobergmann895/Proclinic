...
export class EhrService {
  ...
  async exportPatientBundle(patientId: string, userId: string, tenantId: string) {
    ...
    const bundle = {
      ...
      entry: [
        ...
        ...patient.medications.map((m: any) => this.mapMedicationToFhir(m)),
        ...patient.immunizations.map((i: any) => this.mapImmunizationToFhir(i)),
        ...(patient as any).observations?.map((o: any) => this.mapObservationToFhir(o)) || [],
      ],
    };
  ...
  private async checkAccess(
    ...
    const policy = await (this.prisma as any).accessPolicy?.findFirst({
      where: {
        userId,
        resourceType,
        OR: [
  ...
  async breakTheGlass(
    ...
    await (this.prisma as any).accessPolicy?.create({
      data: {
        userId,
        resourceType,
        resourceId,
